...
if(F){
  ...
  cellstate_list=list('Stat3'=rownames(ABM_cds@phenoData@data[ABM_cds@phenoData@data$final_celltype=='ProE'& ABM_cds@phenoData@data$State=='3',]),
                      'Stat1'=rownames(ABM_cds@phenoData@data[ABM_cds@phenoData@data$final_celltype=='ProE'& ABM_cds@phenoData@data$State=='1',]))
  
  mono_ABM_NRBC_seu$State=ABM_cds@phenoData@data[rownames(mono_ABM_NRBC_seu@meta.data),'State']
  Idents(mono_ABM_NRBC_seu)='State'
  
  ProE_state_marker=FindMarkers(subset(mono_ABM_NRBC_seu,final_celltype =='ProE'),ident.1 = '1',ident.2 = '3')
  
  VlnPlot(mono_ABM_NRBC_seu,features = c('KIT','CD36','GATA2','TFRC','GYPA',"CDK1", "MCM7"),stack =T,split.by = 'State',group.by = 'final_celltype' )
  VlnPlot(mono_ABM_NRBC_seu,features = c('KIT','CD36','GATA2','TFRC','GYPA',"CDK1", "MCM7"),stack =T,split.by = 'State',group.by = 'final_celltype' )
  
  
  plot_cell_trajectory(ABM_cds, markers =  "GYPA",use_color_gradient = T,cell_size = 1)+facet_grid(~final_celltype)+
    plot_cell_trajectory(ABM_cds, markers =  "CDK1",use_color_gradient = T,cell_size = 1)+facet_grid(~final_celltype)
...
new_UCB_NRBC_altas=readRDS("NRBC_UCB_altas/new_UCB_NRBC_altas.rds")
table(new_UCB_NRBC_altas$celltype)

sample_cells=c( sample(rownames(new_UCB_NRBC_altas@meta.data[new_UCB_NRBC_altas$celltype %in% c( 'BFUE/CFUE') ,]),size = 740), # 769 most
                sample(rownames(new_UCB_NRBC_altas@meta.data[new_UCB_NRBC_altas$celltype %in% c( 'ProE') ,]),size = 1231),
                sample(rownames(new_UCB_NRBC_altas@meta.data[new_UCB_NRBC_altas$celltype %in% c( 'Bas') ,]),size = 1500),
                sample(rownames(new_UCB_NRBC_altas@meta.data[new_UCB_NRBC_altas$celltype %in% c( 'Poly') ,]),size = 1500),
                sample(rownames(new_UCB_NRBC_altas@meta.data[new_UCB_NRBC_altas$celltype %in% c( 'Orth') ,]),size = 1500)
)

mono_UCB_NRBC_seu=subset(new_UCB_NRBC_altas,cells=sample_cells)
table(mono_UCB_NRBC_seu$celltype)
rm(new_UCB_NRBC_altas);gc()


pd <- new("AnnotatedDataFrame", data =  mono_UCB_NRBC_seu@meta.data)
fd <- new("AnnotatedDataFrame", data = data.frame(row.names = rownames(mono_UCB_NRBC_seu),gene_short_name=rownames(mono_UCB_NRBC_seu)))

cds <- newCellDataSet(GetAssayData(mono_UCB_NRBC_seu,assay = 'RNA',layer = 'counts'),
                      phenoData =pd,
                      featureData = fd,
                      expressionFamily=negbinomial.size())
...
Idents(mono_UCB_NRBC_seu)='celltype'
marker_gens=FindAllMarkers(mono_UCB_NRBC_seu)
top500_marker_gens=marker_gens %>% group_by( cluster) %>% do(head(.,500))
length(unique(top500_marker_gens$gene)) # 
expressed_genes <- row.names(subset(fData(cds), num_cells_expressed >= 10))
diff_test_res <- differentialGeneTest(cds[expressed_genes,],cores = 6,
...
cds <- reduceDimension(cds, max_components = 2,method = 'DDRTree')
cds <- orderCells(cds)
cds@phenoData@data[,'id']=mono_UCB_NRBC_seu@meta.data[rownames(cds@phenoData@data),'sample']
cds@phenoData@data[,'age']=mono_UCB_NRBC_seu@meta.data[rownames(cds@phenoData@data),'stage']


UCB_cds=cds
saveRDS(UCB_cds,file = 'NRBC_monocle_analysis/sub_UCB_NRBC_cds.rds')

p=plot_cell_trajectory(  UCB_cds, color_by = "celltype",cell_size = 1)+scale_color_manual(values = cols) +ggtitle('UCB NRBC')+
  plot_cell_trajectory(UCB_cds, color_by = "Pseudotime",cell_size = 1)+scale_color_gradient(low ='navy',high ='firebrick3')
p
ggsave(p,filename='NRBC_monocle_analysis/res_pic/UCB_nRBC_celltype_pseudotime_monocle.pdf',width = 10,height = 5)


UCB_cds$Phase =mono_UCB_NRBC_seu@meta.data[rownames(UCB_cds@phenoData@data),'Phase']
p=plot_cell_trajectory(UCB_cds, color_by = 'Phase',cell_size = 0.6)+scale_color_manual(values = cols[-2]) +
  plot_cell_trajectory(UCB_cds, markers =  c("CDK1"),use_color_gradient = T,cell_size = 0.6)
p
ggsave(p,filename='NRBC_monocle_analysis/res_pic/UCB_monocle_cellcycle_analysis.pdf',width =10 ,height =5 ,dpi = 300)
